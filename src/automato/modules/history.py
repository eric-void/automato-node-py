...
def load(entry):
  ...
  entry.history_event_buffer_lock = threading.Lock()
        ...
        for k in entry.history_event_buffer[entry_id][eventname]:
          ...
          for d in entry.history_event_buffer[entry_id][eventname][k]:
            ...
            if t == '#' or d[1]['changed_params']:
              lines.append(utils.strftime(d[1]['time'], '%Y-%m-%d %H:%M:%S.%f') + '\t' + eventname + '\t' + utils.json_export(d[1]['keys']) + '\t' + t + utils.json_export({x: d[1]['params'][x] for x in d[1]['params'] if x not in d[1]['keys']} if t == '#' else d[1]['changed_params']))
            t = ''
