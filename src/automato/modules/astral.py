...
def init(entry):
  try:
    if entry.config['latitude'] is not None:
      entry.location = Location((entry.config['city'], '', entry.config['latitude'], entry.config['longitude'], entry.config['timezone'], entry.config['elevation']))
    else:
      entry.location = Astral()[entry.config['city']]
  except:
